...
1. **Capture live-traffic fixtures (24 h).** Regression-anchor work: capture-middleware tee of southbound POSTs + northbound reads for 24 h, curated into `addon/tests/fixtures/thermostat/live_YYYYMMDD/`. Replaces fixture serial `0000TEST0000` with real-shape fixtures across all mutation kinds and long-tail subpaths. Valuable but **not a prerequisite for the write-path fix** — that fix needs one real config, not a corpus.
   ...
   1a. **Write-path against live config (new slice, unblocked today).** The proxy's `state.db` already holds the real `<system>` config the thermostat posted at cutover (persisted as a BLOB per serial). Extract it, read the relevant `REPLAY_REGISTRY` handler (e.g. the setpoint/hold kinds dispatched by `ZonePatch`), and mentally apply the mutation to the real tree. Diff against the original to identify what the thermostat rejects (likely element order, required attributes we drop, version/hash/timestamp fields, or namespace). Optionally add a ~10-line debug tee in the `GET /config` handler that writes served XML to `design/samples/served_config/` so we confirm theory vs. practice. Update the handler, add a real-shape fixture test, re-run cutover step 7; only then is cutover formally complete.
