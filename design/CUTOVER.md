...
7. **Validate.** Hit `/v1/healthz` — `thermostat.status` should be `healthy`. Confirm `/v1/state` shows accurate live values. Try one PATCH (e.g. `/v1/system/hold` or `/v1/zones/1`) and confirm within ~30 s the thermostat reflects it at the panel.

> **Known issue as of 2026-04-23 cutover:** the write round-trip does **not** land against the real thermostat's config shape. The PATCH applies optimistically to the state store and the pending-write row is marked applied on the next `GET /config`, but the thermostat silently rejects the mutated XML (no matching POST /systems/{serial} "correction" push, subsequent telemetry keeps reporting schedule values). The fixture suite didn't catch this because the mutation works against the fixture config's shape; real-config shape diverges somewhere the thermostat validates. Cutover proceeded **read-only** — reads are faithful, schedule-driven operation works unchanged (use the wall panel for overrides). Write-path fix is gated on live-traffic fixture capture; see "Post-cutover sequencing" item 1 and the backlog's write-path-against-live-config slice.
...
1. **Capture live-traffic fixtures.** *(Promoted to top priority 2026-04-23 — write round-trip against real config failed at cutover step 7; a real config sample is now a prerequisite for fixing writes, not just a regression anchor.)* Immediately after cutover, with the Python proxy live, record another 24 h of southbound POSTs + northbound reads and extend the fixture suite. These are high-value regression anchors for all future slices and are now the gate on the write-path-against-live-config slice below.

   1a. **Write-path against live config (new slice).** Once live fixtures are captured, diff the real `<system>`/`<zone>` XML shape against the boot_01 fixture. Update `mutations.REPLAY_REGISTRY` handlers so the emitted mutation passes whatever the thermostat validates (likely element order, required attributes, version/hash fields, or namespace). Add a fixture-based regression test for each mutation kind using the real-shape fixture. Don't declare cutover step 7 green until a PATCH reflects at the panel.
