...
`;var Et=class extends w{static properties={...w.baseProperties,_whHoldOpen:{state:!0},_whHoldActivity:{state:!0},_whHoldDuration:{state:!0},_whHoldCustom:{state:!0}};constructor(){super(),this._whHoldOpen=!1,this._whHoldActivity="home",this._whHoldDuration="120",this._whHoldCustom=""}static getStubConfig(){return{}}getCardSize(){return 3}static styles=[O,E`
    .card-pad { padding: 16px; }
    .header {
      display: flex; align-items: center; gap: 8px; margin-bottom: 14px;
    }
    .header-title { font-size: 18px; font-weight: 500; color: var(--primary-text-color); }
  `];render(){if(!this.hass)return u;if(!this._registryLoaded)return this._renderLoading();let t=this._findEntities(),{system:e,selects:s,climates:i}=t,o=i.length&&this._st(i[0])?.state||"off",a=["off","heat","cool","heat_cool"],d={off:"Off",heat:"Heat",cool:"Cool",heat_cool:"Auto"},c=this._st(e.oat),n="\u2013";if(c?.state&&c.state!=="unavailable")n=`${Math.round(Number(c.state))}\xB0`;else if(i.length){let f=this._at(i[0],"outdoor_temperature");f!=null&&(n=`${Math.round(Number(f))}\xB0`)}let p=this._st(e.opStatus),h=p?.state&&p.state!=="unavailable"?p.state:"",m=this._st(e.humidifier)?.state==="on",v=this._st(s.wholeHouse),_=v&&v.state!=="off",z=i.length?this._at(i[0],"whole_house_hold_until"):null,A=i.length?this._at(i[0],"current_humidity"):null,H=e.info?this._st(e.info)?.state!=="unavailable":!1,C=e.info?this._at(e.info,"sse_connected"):null,M=e.info?this._at(e.info,"carrier_status"):null,[b,P]=this._infinitudeDot(H,C),[N,T]=this._carrierDot(M);return r`
      <ha-card>
        <div class="card-pad">
          <div class="header">
            <span class="header-title">infinitude</span>
            <span class="conn-dot ${b}" title="${P}"></span>
            <span class="conn-dot ${N}" title="${T}"></span>
          ...
          ${_?r`
            <div class="wh-hold" @click=${()=>this._cancelWholeHouseHold()}>
              ...
              <span>Whole house: <strong>${v.state}</strong>${z?r` · <span style="opacity:0.85">${this._otmrRelative(z)}</span>`:u}</span>
  ...
  `];render(){if(!this.hass)return u;if(!this._registryLoaded)return this._renderLoading();let t=this._config.entity;if(!t)return r`<ha-card><div class="card-pad" style="color:var(--secondary-text-color)">No entity configured</div></ha-card>`;let e=this._st(t);if(!e)return r`<ha-card><div class="card-pad" style="color:var(--secondary-text-color)">Entity unavailable</div></ha-card>`;let s=e.attributes||{},i=this._zoneId(t),o=k[parseInt(i)-1]||i,a=(s.friendly_name||t).replace(/^Infinitude\s+/i,"").replace(/^infinitude_direct\s+/i,""),d=o?`${o} ${a}`:a,c=s.current_temperature!=null?Math.round(s.current_temperature):"\u2013",n=s.current_humidity,p=e.state||"off",h=s.hvac_action||"idle",m=s.preset_mode||"\u2013",v=h==="heating"?"heating":h==="cooling"?"cooling":h==="drying"?"drying":"",_=h==="idle"?"Idle":h.charAt(0).toUpperCase()+h.slice(1),z=p==="heat_cool",A=!!s.hold_active,H=s.hold_activity||m,C=s.hold_until,M=this._pendingTemps[t],b=this._hasPending(t),P=M?.heat??s.target_temp_low??s.temperature??null,N=M?.cool??s.target_temp_high??(p==="cool"?s.temperature:null)??null;return r`
      <ha-card class="${v}">
        <div class="card-pad">
          <div class="zone-top">
            <div style="display:flex;align-items:center;gap:8px">
              <span class="zone-cond-dot ${v||"idle"}" title="${h==="drying"?"Dehumidifying":_}"></span>
            ...
            <div class="zone-sp">
              ${P!=null?r`
                <div class="sp-row">
                  ...
                  <span class="sp-val sp-heat ${b?"sp-pending":""}">${Math.round(P)}°</span>
                  <button class="btn-adj" @click=${()=>this._adjustTemp(t,1,"heat")}>+</button>
                </div>`:u}
              ${z&&N!=null?r`
                <div class="sp-row">
                  <button class="btn-adj" @click=${()=>this._adjustTemp(t,-1,"cool")}>−</button>
                  <span class="sp-val sp-cool ${b?"sp-pending":""}">${Math.round(N)}°</span>
                  <button class="btn-adj" @click=${()=>this._adjustTemp(t,1,"cool")}>+</button>
                </div>`:u}
            </div>
          </div>
          <div class="zone-meta">
            ...
            ${v&&s.conditioning_stage!=null?r`<span class="meta-item ${v}">Stage <span class="meta-val">${s.conditioning_stage}</span></span>`:u}
          </div>
          ${A?r`
            <div class="zone-hold">
              <span class="hold-label">Hold: ${H}${C?` \xB7 ${this._otmrRelative(C)}`:""}</span>
    ...
    `}};customElements.get("infinitude-zone-card-editor")||customElements.define("infinitude-zone-card-editor",Ct);customElements.get("infinitude-zone-card")||customElements.define("infinitude-zone-card",kt);window.customCards=window.customCards||[];window.customCards.some(l=>l.type==="infinitude-zone-card")||window.customCards.push({type:"infinitude-zone-card",name:"Infinitude Zone",description:"Single zone control for Carrier/Bryant Infinity thermostat",preview:!1});var St=class extends w{static properties={...w.baseProperties,_schedDay:{state:!0},_schedEdits:{state:!0}};constructor(){super(),this._schedDay=S[K[new Date().getDay()]],this._schedEdits={},this._saving=!1}static getStubConfig(){return{}}getCardSize(){return 6}static styles=[O,E`
    .card-pad { padding: 16px; }
  ...
  `];render(){if(!this.hass)return u;if(!this._registryLoaded)return this._renderLoading();let t=this._getScheduleData(),e=this._getProfilesData(),s=Object.keys(t);if(!s.length)return r`<ha-card>
      ...
      </div></ha-card>`;let i=S[K[new Date().getDay()]],o={},a={};for(let n=0;n<e.length;n++)o[e[n].id]=e[n].name,a[e[n].id]=n;let d=0;for(let n of s)d=Math.max(d,(t[n]?.[this._schedDay]||[]).length);d||(d=5);let c=Object.keys(this._schedEdits).length>0;return r`
      <ha-card>
        <div class="card-pad">
          <div class="section-title">Schedule</div>
          ${s.length>1?r`<div class="zone-legend">${s.map((n,p)=>r`<span class="legend-item"><span class="legend-num">${k[p]||p+1}</span> ${o[n]||`Zone ${n}`}</span>`)}</div>`:u}
  ...
  `];render(){if(!this.hass)return u;if(!this._registryLoaded)return this._renderLoading();let t=this._getProfilesData();if(!t.length)return r`<ha-card>
      ...
      <ha-card>
        <div class="card-pad">
          ...
          ${e?r`<div class="zone-legend">${t.map((i,o)=>r`<span class="legend-item"><span class="legend-num">${k[o]||o+1}</span> ${i.name}</span>`)}</div>`:u}
          ${L.map(i=>r`
            <div class="prof-card">
              <div class="prof-header">${i}</div>
              ${t.map((o,a)=>{let d=o.activities?.[i]||{},c=`${o.id}_${i}`,n=this._profileEdits[c],p=n?.htsp??(d.htsp?Math.round(Number(d.htsp)||x):x),h=n?.clsp??(d.clsp?Math.round(Number(d.clsp)||$):$),m=n?.fan??d.fan??"low",v=e?k[a]||a+1:o.name;return r`
                  <div class="prof-line">
  ...
  `];render(){if(!this.hass)return u;if(!this._registryLoaded)return r`<ha-card>
      ...
      <div class="zone-grid">${e.map(s=>this._zoneCard(s))}</div>`:r`<div style="padding:20px;text-align:center;color:var(--secondary-text-color)">No zone entities found</div>`}_summaryStrip(t){let{system:e,selects:s,climates:i}=t,o=i.length&&this._st(i[0])?.state||"off",a=["off","heat","cool","heat_cool","fan_only"],d={off:"Off",heat:"Heat",cool:"Cool",heat_cool:"Auto",fan_only:"Fan"},c=this._st(e.oat),n="\u2013";if(c?.state&&c.state!=="unavailable")n=`${Math.round(Number(c.state))}\xB0`;else if(i.length){let g=this._at(i[0],"outdoor_temperature");g!=null&&(n=`${Math.round(Number(g))}\xB0`)}let p=this._st(e.opStatus),h=p?.state&&p.state!=="unavailable"?p.state:"",m=this._st(e.humidifier)?.state==="on",v=this._st(s.wholeHouse),_=v&&v.state!=="off",z=i.length?this._at(i[0],"whole_house_hold_until"):null,A=i.length?this._at(i[0],"current_humidity"):null,H=i.some(g=>this._at(g,"hvac_action")==="heating"),C=i.some(g=>this._at(g,"hvac_action")==="cooling"),M=i.some(g=>this._at(g,"hvac_action")==="drying"),b=H?"Heating":C?"Cooling":M?"Dehumidifying":h||"Idle",P=H?"heat":C?"cool":"",N=this._registryEntities||[],T=0;for(let g of N){let y=this.hass.states[g.entity_id]?.last_reported;if(!y)continue;let Mt=Date.parse(y);Mt>T&&(T=Mt)}let f=T>0&&Date.now()-T>9e4;return r`
      <div class="mode-row">
        <span class="mode-label">Mode</span>
        <div class="mode-pills">
      ...
      <div class="summary-stats">
        <div class="summary-stat">
          <span class="summary-stat-label">Status</span>
          <span class="summary-stat-val ${P}">${b}</span>
        </div>
        <div class="summary-divider"></div>
        <div class="summary-stat">
          <span class="summary-stat-label">Outdoor</span>
      ...
      ${f?r`<div class="stale-warn">⚠ Thermostat data is stale — system may be offline</div>`:u}
      ${_?r`
        <div class="wh-hold" @click=${()=>this._cancelWholeHouseHold()}>
          <ha-icon icon="mdi:home-lock" style="--mdc-icon-size:16px"></ha-icon>
          <span>Whole house: <strong>${v.state}</strong>${z?r` · <span style="opacity:0.85">${this._otmrRelative(z)}</span>`:u}</span>
      ...
      `}`}_zoneCard(t){let e=this._st(t);if(!e)return u;let s=e.attributes||{},i=this._zoneId(t),o=k[parseInt(i)-1]||i,a=(s.friendly_name||t).replace(/^Infinitude\s+/i,"").replace(/^infinitude_direct\s+/i,""),d=o?`${o} ${a}`:a,c=s.current_temperature!=null?Math.round(s.current_temperature):"\u2013",n=s.current_humidity,p=e.state||"off",h=s.hvac_action||"idle",m=s.preset_mode||"\u2013",v=h==="heating"?"heating":h==="cooling"?"cooling":h==="drying"?"drying":"",_=h==="idle"?"Idle":h.charAt(0).toUpperCase()+h.slice(1),z=p==="heat_cool",A=!!s.hold_active,H=s.hold_activity||m,C=s.hold_until,M=this._holdOpen===t,b=this._pendingTemps[t],P=this._hasPending(t),N=b?.heat??s.target_temp_low??s.temperature??null,T=b?.cool??s.target_temp_high??(p==="cool"?s.temperature:null)??null;return r`
      <div class="zone-card ${v}">
        <div class="zone-top">
          <div style="display:flex;align-items:center;gap:8px">
            <span class="zone-cond-dot ${v||"idle"}" title="${h==="drying"?"Dehumidifying":_}"></span>
            <span class="zone-name">${d}</span>
        ...
        <div class="zone-body">
          ...
          <div class="zone-sp">
            ${N!=null?r`
              <div class="sp-row">
                <button class="btn-adj" @click=${()=>this._adjustTemp(t,-1,"heat")}>−</button>
                <span class="sp-val sp-heat ${P?"sp-pending":""}">${Math.round(N)}°</span>
                <button class="btn-adj" @click=${()=>this._adjustTemp(t,1,"heat")}>+</button>
              </div>`:u}
            ${z&&T!=null?r`
              <div class="sp-row">
                <button class="btn-adj" @click=${()=>this._adjustTemp(t,-1,"cool")}>−</button>
                <span class="sp-val sp-cool ${P?"sp-pending":""}">${Math.round(T)}°</span>
                <button class="btn-adj" @click=${()=>this._adjustTemp(t,1,"cool")}>+</button>
              </div>`:u}
          </div>
        </div>
      ...
      </div>`;let i=S[K[new Date().getDay()]],o={},a={};for(let n=0;n<e.length;n++)o[e[n].id]=e[n].name,a[e[n].id]=n;let d=0;for(let n of s)d=Math.max(d,(t[n]?.[this._schedDay]||[]).length);d||(d=5);let c=Object.keys(this._schedEdits).length>0;return r`
      <div class="section-title">Schedule</div>
      ${s.length>1?r`<div class="zone-legend">${s.map((n,p)=>r`<span class="legend-item"><span class="legend-num">${k[p]||p+1}</span> ${o[n]||`Zone ${n}`}</span>`)}</div>`:u}
      <div class="sched-day-tabs">
        ${S.map(n=>r`
